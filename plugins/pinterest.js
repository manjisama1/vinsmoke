...
const getBuf = async (url, attempt = 1) => {
  try {
    ...
    return attempt <= 2 ? getBuf(url, attempt + 1) : null;
  }
};
...
Command({
  ...
  const handle = async (links, mode, isQueue = false) => {
    ...
    try {
      for (const url of links) {
        ...
        if (stic || buf) {
          const ok = stic
            ? await message.send({ sticker: stic }).catch(() => null)
            : await message.send(buf).catch(() => null);
  ...
  if (!match && isDoc) {
    ...
    const links = fs.readFileSync(fpath, 'utf-8').match(/https?:\/\/[^\s]+/g) || [];
    if (!links.length) {
      del(fpath);
      return message.send(lang.plugins.pinterest.no_links);
    }

    const { sent, stop } = await handle(links, 's');
    del(fpath);

    return stop
      ? message.send(lang.plugins.pinterest.stopped.format(sent, links.length))
  ...
  if (cmd === 'go') {
    ...
    const { sent, stop } = await handle(q.links, q.type, true);
    return stop
      ? message.send(lang.plugins.pinterest.stopped.format(sent, q.links.length))
  ...
  const { sent, stop } = await handle(links, mode);
